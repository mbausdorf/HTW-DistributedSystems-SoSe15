...
/**
 * Demonstrates thread processing and thread resynchronization using a runnable in conjunction with
 * blocking, and a situationally correct way of handling {@linkplain InterruptedException}. Note
 * that there is no "golden" way to handle thread interruption which is always correct, i.e. the
 * handling must always take circumstances into account! However, an interruption MUST always be
 * handled immediately, in one or more of the following ways:
 * <ul>
 ...
 * Especially note that the expression {@code Thread.currentThread().join()} is the easiest way to
 * produce a guaranteed deadlock! Finally, note that this class is declared final because it
...
public final class ResyncThreadByBlocking {
	...
	/**
	* CHOOSING to re-interrupt the NEXT blocking operation AFTER resynchronization - see class comment!
	...
	* @param thread the thread to resynchronize into the current thread
	* @throws IllegalArgumentException if the given thread is the current thread
	*/
	static private void joinUninterruptibly (final Thread thread) {
		if (thread == Thread.currentThread()) throw new IllegalArgumentException();
